...
export async function createTodo(userId: number, todo: CreateTodoInput) {
  ...
  const response = await ky.post(`${import.meta.env.VITE_API_URL}/todos/${userId}`, {
    json: todo,
    headers: {
      Authorization: `Bearer ${token}`,
    },
  ...
  return await response.json<{ item: Todo }>();
}

export async function updateTodo(id: number, todo: UpdateTodoInput) {
  const token = localStorage.getItem("token");

  const response = await ky.put(`${import.meta.env.VITE_API_URL}/todos/${id}`, {
    json: todo,
    headers: {
      Authorization: `Bearer ${token}`,
    },
...
export async function deleteTodo(id: number) {
  ...
  const response = await ky.delete(`${import.meta.env.VITE_API_URL}/todos/${id}`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
